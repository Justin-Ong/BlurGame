	...
	public class AdvancedWalkerController : Controller {
		...
		protected virtual Vector3 CalculateMovementVelocity()
		{
			...
			if (_velocity == Vector3.zero && isHovering)
			{
				...
				if (Mathf.Approximately(mover.GetGroundNormal().x, 0))
				{
					if (Mathf.Approximately(savedVelocity.x, 0))
					{
						_velocity = Vector3.zero;
					}
					else
					{
						_velocity = new Vector3(prevDir, 0, 0);
					}
				}
				else
				{
					_velocity = new Vector3(slopeDir, 0, 0);
				}
			}
			else
			{
